...
private:
  ...
  bool checkFirstAnswer(std::string &input, User &user,
                        ...
                        double &sum, int &duration, bool &withdrawProcents);
  ...
  bool showIfMenu(User &user, double sum, int duration, bool withdrawProcents,
                  Deposit &recommendedDeposit, double newBet);
  void increaseInterestRate(const User &user) const;

public:
  explicit DepositAssistant(BankSystem &bank);
  void runAssistant(User &user, const std::vector<Deposit> &availableDeposits);
  void DepositChoice(User &user, const std::vector<Deposit> &availableDeposits);
};
