...
bool UserDepositManager::syncUserDeposits(const User &user) {
  ...
  auto response =
      bankServer->get_deposits(1, 36, 0, 1000, 0, user.getSalary(),
                               user.getBroker(), user.getSubscription());
...
std::vector<Deposit>
UserDepositManager::getAvailableDeposits(const User &user) {
...
void UserDepositManager::setAvailableDeposits(
    const User &user, const std::vector<Deposit> &deposits) {
