...
Server::Response<User> Server::sign_up(const std::string &email,
  ...
  try {
    ...
    auto r = cpr::Post(cpr::Url{url + "/sign_up"},
    ...
    if (r.status_code == 200) {
      ...
      if (jResponse["code"] == 0) {
        ...
        User user(jUser["id"], jUser["name"], jUser["email"], jUser["salary"],
                  jUser["broker"], jUser["sub"]);
...
Server::Response<User> Server::sign_in(const std::string &email,
                                       const std::string &password) {
  try {
    json requestBody = {{"email", email}, {"password", password}};

    auto r = cpr::Post(cpr::Url{url + "/sign_in"},
                       cpr::Header{{"Content-Type", "application/json"}},
                       cpr::Body{requestBody.dump()});
...
Server::Response<std::vector<Deposit>>
Server::get_deposits(double duration_min, double duration_max,
                     double percent_min, double percent_max, long long sum_min,
                     bool reqSalary, bool reqBroker, bool reqPremium) {
  try {
    auto r = cpr::Get(
        cpr::Url{url + "/get_deposits"},
        cpr::Header{{"Content-Type", "application/json"}},
        cpr::Parameters({{"duration_min", std::to_string(duration_min)},
    ...
    if (r.status_code == 200) {
      ...
      for (auto jDeposit : jResponse["data"]) {
        deposits.push_back(
            Deposit(jDeposit["id"], jDeposit["name"], jDeposit["description"],
                    jDeposit["duration_min"], jDeposit["duration_max"],
                    jDeposit["percent_min"], jDeposit["percent_max"],
                    jDeposit["sum_min"], jDeposit["req_salary"],
                    jDeposit["req_broker"], jDeposit["req_premium"]));
      }
      return Response<std::vector<Deposit>>(true, jResponse["code"], deposits);
    } else {
      return Response<std::vector<Deposit>>(false, r.status_code,
                                            std::vector<Deposit>());
    }
  } catch (const std::exception &e) {
    std::cerr << "Server error occurred: " << e.what() << std::endl;
...
Server::Response<User> Server::load_user() {
  try {
    auto r = cpr::Get(cpr::Url{url + "/get_user"},
    ...
    if (r.status_code == 200) {
      ...
      if (jResponse["code"] == 0) {
        ...
        return Response<User>(true, 0,
                              User(user["id"], user["name"], user["email"],
                                   user["salary"], user["broker"],
                                   user["sub"]));
      } else {
...
Server::Response<> Server::save_user(const User &user) {
  try {
    ...
    json requestBody = {{"name", user.getName()},
                        {"salary", user.getSalary()},
                        {"broker", user.getBroker()},
                        {"sub", user.getSubscription()}};

    auto r = cpr::Post(cpr::Url{url + "/update_user"},
                       cpr::Header{{"Authorization", "Bearer " + token},
